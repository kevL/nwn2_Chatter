...
namespace NAudio.Wave
{
	...
	/// </summary>
	public class WaveFormatConversionProvider
...
//		public WaveFormat WaveFormat { get; }
		public WaveFormat WaveFormat { get; set; } // kL
...
//				throw new ArgumentException("Must read complete blocks");
			...
			while (bytesRead < count)
			{
				...
				{
					Array.Copy(conversionStream.DestBuffer,
							   leftoverDestOffset,
							   buffer,
							   offset+bytesRead,
				...
				// always read our preferred size, we can always keep leftovers
				// for the next call to Read if we get too much
				int sourceBytesRead = sourceProvider.Read(conversionStream.SourceBuffer,
														  leftoverSourceBytes,
														  sourceReadSize);
...
//					Debug.WriteLine($"Warning: couldn't convert anything from {sourceBytesAvailable}");
					Debug.WriteLine( "Warning: couldn't convert anything from {sourceBytesAvailable}"); // kL
				...
				if (leftoverSourceBytes > 0)
				{
					...
					Buffer.BlockCopy(conversionStream.SourceBuffer,
									 sourceBytesConverted,
									 conversionStream.SourceBuffer,
									 0,
									 leftoverSourceBytes);
				}
...
//						$"sourceBytesRead: {sourceBytesRead}, sourceBytesConverted {sourceBytesConverted}, destBytesConverted {destBytesConverted}");
						 "sourceBytesRead: {sourceBytesRead}, sourceBytesConverted {sourceBytesConverted}, destBytesConverted {destBytesConverted}"); // kL
