...
namespace NAudio.Wave
{
		...
		: WaveStream
	{
		...
		/// <summary>
		/// The MP3 wave format (n.b. NOT the output format of this stream - see
		/// the WaveFormat property)
		/// </summary>
...
//			new DmoMp3FrameDecompressor(this.Mp3WaveFormat);
		...
		{
			get { return totalSamples * bytesPerSample; }
		}
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			lock (repositionLock)
			{
				...
				{
					int toCopy = Math.Min(decompressLeftovers, count);
				...
				while (bytesRead < count)
				{
					...
					if (frame != null)
					{
						...
						int toCopy = Math.Min(decompressed - decompressBufferOffset, count - bytesRead);
						Array.Copy(decompressBuffer, decompressBufferOffset, buffer, offset, toCopy);
			...
			Debug.Assert(bytesRead <= count, "MP3 File Reader read too much");
			position += bytesRead;
			return bytesRead;
		}
