		...
		: IDisposable
	{
		...
		public AcmStream(WaveFormat sourceFormat, WaveFormat destFormat)
		{
			try
			{
				...
				try
				{
					MmException.Try(AcmInterop.acmStreamOpen2(out streamHandle,
															  IntPtr.Zero,
															  sourceFormatPointer,
															  destFormatPointer,
															  null,
		...
		public int SourceToDest(int source)
		{
			if (source == 0) // zero is an invalid parameter to acmStreamSize
				return 0;
			int convertedBytes;
			var mmResult = AcmInterop.acmStreamSize(streamHandle,
													source,
													out convertedBytes,
		...
		public int DestToSource(int dest)
		{
			if (dest == 0) // zero is an invalid parameter to acmStreamSize
				return 0;
			int convertedBytes;
			MmException.Try(AcmInterop.acmStreamSize(streamHandle,
													 dest,
													 out convertedBytes,
													 AcmStreamSizeFlags.Destination), "acmStreamSize");
			return convertedBytes;
		}
...
//			MmException.Try(AcmInterop.acmFormatSuggest(IntPtr.Zero,
//														compressedFormat,
//														suggestedFormat,
//														Marshal.SizeOf(suggestedFormat),
//														AcmFormatSuggestFlags.FormatTag), "acmFormatSuggest");
			...
			try
			{
				MmResult result = AcmInterop.acmFormatSuggest2(IntPtr.Zero,
															   compressedFormatPointer,
															   suggestedFormatPointer,
															   Marshal.SizeOf(suggestedFormat),
