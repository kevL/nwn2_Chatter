...
/*		/// <summary>
		...
		public void WriteSample(float sample)
		{
			...
			{
				_writer.Write((Int16)(Int16.MaxValue * sample));
				dataChunkSize += 2;
			}
...
/*		/// <summary>
		...
		public void WriteSamples(short[] samples, int offset, int count)
		{
			...
			else if (WaveFormat.BitsPerSample == 32 && WaveFormat.Encoding == WaveFormatEncoding.Extensible)
			{
				for (int sample = 0; sample < count; sample++)
				{
					_writer.Write(UInt16.MaxValue * (Int32)samples[sample + offset]);
				}
			...
			else if (WaveFormat.BitsPerSample == 32 && WaveFormat.Encoding == WaveFormatEncoding.IeeeFloat)
			{
				for (int sample = 0; sample < count; sample++)
				{
					_writer.Write((float)samples[sample + offset] / (float)(Int16.MaxValue + 1));
				}
				dataChunkSize += (count * 4);
			}
			else
		...
		/// <summary>
		/// Finaliser - should only be called if the user forgot to close this
		/// WaveFileWriter
		/// </summary>
		~WaveFileWriter()
		{
