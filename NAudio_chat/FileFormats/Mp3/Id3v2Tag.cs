...
namespace NAudio.Wave
{
	...
	public class Id3v2Tag
	{
		...
		static byte[] CreateId3v2Frame(string key, string value)
		{
			...
			if (key == "COMM") // comment
			{
				body = ByteArrayExtensions.Concat(new byte[] { UnicodeEncoding },
												  language,
												  shortDescription,
												  UnicodeOrder,
			...
			return ByteArrayExtensions.Concat(Encoding.UTF8.GetBytes(key),
		...
		/// <summary>
		/// Gets the Id3v2 Header size. The size is encoded so that only 7 bits
		/// per byte are actually used.
		/// </summary>
		...
		static byte[] CreateId3v2TagHeader(IEnumerable<byte[]> frames)
		{
			...
			byte[] tagHeader = ByteArrayExtensions.Concat(Encoding.UTF8.GetBytes("ID3"),
